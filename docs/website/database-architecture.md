...
* **`MissingLookup`** - records when a lookup couldn't be completed because something was missing (such as a language, structure, or concept that doesn't exist yet). It records the type of item that was missing and its value, and links back to the `SiteVisit` where it happened. This data feeds into the issues the team files for missing information.
